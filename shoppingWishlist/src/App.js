...
export default class App extends Component {
  ...
  addProduct = (name,price,link,quantity) => {
    api.add(name,parseFloat(price),link,quantity).then(resp => {
      const newProduct = {"id":resp.id, "priority": resp.priority, "name":name, "price":price,"link":link,"quantity": quantity };
      this.setState({name: this.state.name, minPrice: this.state.minPrice, maxPrice: this.state.maxPrice, order: this.state.order, products: this.state.products.concat([newProduct])});
    })
  };
